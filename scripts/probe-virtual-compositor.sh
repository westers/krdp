...
if [[ "${3:-}" == --worker || "${3:-}" == --multi-worker || "${3:-}" == --multi-mixed || "${3:-}" == --multi-create || "${3:-}" == --multi-add || "${3:-}" == --multi-mixed-create || "${3:-}" == --multi-remove || "${3:-}" == --multi-resize || "${3:-}" == --multi-fit || "${3:-}" == --multi-primary || "${3:-}" == --multi-negative || "${3:-}" == --multi-window || "${3:-}" == --multi-input || "${3:-}" == --multi-drag || "${3:-}" == --multi-reposition ]]; then
    ...
    [[ "${3:-}" != --multi-mixed-create ]] || probe_args=(--multi-mixed-create)
    ...
    kill -0 "$wrapper_pid"
    kill -0 "$plasma_pid"
    if [[ "${3:-}" == --multi-add || "${3:-}" == --multi-mixed-create || "${3:-}" == --multi-remove ]]; then
    ...
    elif [[ "${3:-}" == --multi-mixed-create ]]; then
        jq -e 'any(.outputs[]; .name == "Virtual-0" and .priority == 1 and .size.width == 1280 and .size.height == 720 and .pos.x == 0 and .pos.y == 0) and any(.outputs[]; .name == "Virtual-1" and .size.width == 1280 and .size.height == 720 and .pos.x == 1280 and .pos.y == 100)' "$XDG_RUNTIME_DIR/outputs-after-worker.json"
    else
