...
- Coordinates (AMENDED 2026-09-16 after Task 3): the RDP desktop space is the bounding union of the monitors with the UNION's top-left at `(0,0)` — never negative, because `MapSurfaceToOutput.outputOriginX/Y` are UINT32 on the wire and FreeRDP's own shadow server sends X11 (union-anchored) coordinates in `RESET_GRAPHICS`. The primary is marked only by `MONITOR_PRIMARY`. KWin-global coordinates are the `QScreen::geometry()` values. Conversion is a constant translation `rdp = kwin − union.topLeft()` (and back `kwin = rdp + union.topLeft()` for input); keep it in exactly one helper (`SurfaceLayout`). The previous "primary at (0,0)" rule was an mstsc convention; the client is a FreeRDP 3 client on Kubuntu, not mstsc.
