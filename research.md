...
- OPT-040 PLANNED (after OPT-037) — client-layout mapping for third-party `/multimon` clients: when the client declares N monitors, advertise the *client's* layout in `ResetGraphics` and map each server surface onto it with RDPGFX `MapSurfaceToScaledOutput` (needs caps ≥ 10.3, so also revisit `VideoStream::onCapsAdvertise()`, which currently insists on the YUV420 flag and therefore picks 8.1 from FreeRDP clients), scaling input coordinates back accordingly. See the `/multimon` finding above.
